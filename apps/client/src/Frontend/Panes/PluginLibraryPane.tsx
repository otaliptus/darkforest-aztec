...
export function PluginLibraryPane({
  ...
  const [forceReloadEmbeddedPlugins, _s] = useBooleanSetting(
    gameUIManager,
    forceReloadEmbeddedPluginsSetting ?? Setting.HasAcceptedPluginRisk
  );
  const shouldForceReloadEmbeddedPlugins = forceReloadEmbeddedPluginsSetting
    ? forceReloadEmbeddedPlugins
    : false;
   ...
   */
  useEffect(() => {
    pluginManager.load(isAdmin, shouldForceReloadEmbeddedPlugins);
  }, [pluginManager, isAdmin, shouldForceReloadEmbeddedPlugins]);
