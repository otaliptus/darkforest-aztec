...
export type LobbyConfigAction =
  | { type: 'SPACESHIPS'; value: Record<string, boolean> | undefined }
...
export type LobbyInitializers = Initializers & {
  ...
  SPACESHIPS?: Record<string, boolean>;
  ROUND_END_REWARDS_BY_RANK?: number[];
};

export type LobbyConfigState = Record<string, any>;
...
export function ofSpaceJunkLimit(
  ...
  if (state.PLANET_LEVEL_JUNK.currentValue) {
    ...
    state.PLANET_LEVEL_JUNK.currentValue.forEach((junkAmount: number, planetLevel: number) => {
      if (value < junkAmount) {
