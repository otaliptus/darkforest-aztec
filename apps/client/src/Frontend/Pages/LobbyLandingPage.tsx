import { ThrottledConcurrentQueue, weiToEth } from '@darkforest_eth/network';
...
class LobbyPageTerminal {
  private ethConnection: AztecConnection;
...
export function LobbyLandingPage({
  ...
  onReady: (connection: AztecConnection) => void;
}) {
  ...
  }, [terminal, connection, controller, onReady, isAztec]);
    ...
    return (
      <TerminalWrapper initRender={InitRenderState.NONE} terminalEnabled={false}>
        ...
        <div style={{ color: 'white', padding: '16px' }}>
          Lobbies are not supported on the Aztec client. Please return to the main game.
        </div>
      </TerminalWrapper>
    );
  }
