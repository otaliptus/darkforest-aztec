...
import { ArtifactRarity, ContractMethodName, EthAddress, UnconfirmedCreateLobby } from '@darkforest_eth/types';
...
export function CreateLobby({ match }: RouteComponentProps<{ contract: string }>) {
  const [connection, setConnection] = useState<AztecConnection | undefined>();
  ...
  const [errorState, setErrorState] = useState<ErrorState | undefined>(
    isAztec ? undefined : contractAddress ? undefined : { type: 'invalidAddress' }
  );
  ...
  }, [connection, contractAddress, isAztec]);
    ...
    if (contract) {
      contract
        ...
        .then((config) => {
          ...
          } as LobbyInitializers;
          setStartingConfig(startingConfig);
        })
        .catch((e) => {
          console.log(e);
  ...
  if (isAztec) {
    return (
      <Wrapper initRender={InitRenderState.NONE} terminalEnabled={false}>
        <CadetWormhole imgUrl='/public/img/no-contract-text.png' />
      </Wrapper>
    );
  }
