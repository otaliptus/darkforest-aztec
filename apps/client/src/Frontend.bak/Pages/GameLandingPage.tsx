...
import { CLIENT_CONFIG, MAX_TEST_ACCOUNTS } from '../../Backend/Aztec/config';
...
export function GameLandingPage({ match, location }: RouteComponentProps<{ contract: string }>) {
  ...
  const [ethConnection, setEthConnection] = useState<AztecConnection | undefined>();
  ...
  const accountIndexParam = params.get('accountIndex');
  const isAztec = true;
  const contractAddress = (CLIENT_CONFIG.darkforestAddress || match.params.contract) as EthAddress;
  const isLobby = false;

  if (accountIndexParam) {
    const parsedIndex = Number(accountIndexParam);
    if (!Number.isNaN(parsedIndex)) {
      CLIENT_CONFIG.account = {
        ...CLIENT_CONFIG.account,
        testAccountIndex: parsedIndex,
      };
    }
  }
  ...
  useEffect(() => {
    getEthConnection()
      ...
      .catch((e) => {
        ...
        const hint = message.includes('No initial test account')
          ? 'Only accountIndex 0-2 exist on local devnet. Redeploy contracts if all are used.'
          : undefined;
        alert(
          `Error connecting to Aztec node: ${message}${hint ? `\n${hint}` : ''}`
        );
        console.error(e);
      });
  }, []);
  ...
  useEffect(() => {
    ...
    gameManager.startSnapshotLiveUpdates();
  }, [gameManager]);
  ...
  const advanceStateFromAccountSet = useCallback(
    ...
    [ethConnection, isProd, contractAddress, isAztec]
  );
  ...
  const advanceStateFromAskWhitelistKey = useCallback(
    async (terminal: React.MutableRefObject<TerminalHandle | undefined>) => {
      ...
      if (!useZkWhitelist) {
        ...
        const keyBigInt = bigInt(key.replace(/-/g, ''), 16);
  ...
  const advanceStateFromFetchingEthData = useCallback(
    async (terminal: React.MutableRefObject<TerminalHandle | undefined>) => {
      ...
      try {
        ...
        terminal.current?.println(
          'Network under heavy load. Please refresh the page and check your Aztec node logs.',
          TerminalTextStyle.Red
        );
  ...
  const advanceStateFromAskAddAccount = useCallback(
    async (terminal: React.MutableRefObject<TerminalHandle | undefined>) => {
      const currentIndex = CLIENT_CONFIG.account?.testAccountIndex ?? 0;
      const maxIndex = Math.max(MAX_TEST_ACCOUNTS - 1, 0);
      ...
      if (isAztec) {
        terminal.current?.println(
          `Tip: press ENTER or type 'n' to switch to the next local Aztec test account (current: ${currentIndex}, max: ${maxIndex}).`,
          TerminalTextStyle.Sub
        );
      }
      const rawInput = (await terminal.current?.getInput()) ?? '';
      const normalizedInput =
        rawInput.trim().toLowerCase() === '' && isAztec ? 'n' : rawInput.trim().toLowerCase();
      if (normalizedInput === 'y') {
        ...
        if (isAztec) {
          if (currentIndex >= maxIndex) {
            terminal.current?.println(
              `ERROR: All ${MAX_TEST_ACCOUNTS} local Aztec test accounts are already initialized on this contract.`,
              TerminalTextStyle.Red
            );
            terminal.current?.println(
              'Fix: redeploy local contracts or reset the local node data, then reload the client.',
              TerminalTextStyle.Sub
            );
            terminal.current?.println(
              'Suggested: node packages/contracts/scripts/deploy_local.js --write-env --overwrite-env',
              TerminalTextStyle.Sub
            );
            setStep(TerminalPromptStep.TERMINATED);
            return;
          ...
          terminal.current?.println(
            `Switching to test account index ${nextIndex} and reloading...`
          );
          const nextUrl = new URL(window.location.href);
          nextUrl.searchParams.set('accountIndex', String(nextIndex));
