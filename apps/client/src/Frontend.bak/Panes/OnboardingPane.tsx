  ...
  return (
    <StyledOnboardingContent>
      ...
      <p>
        {isAztec ? (
          <>
            <Red>Aztec mode:</Red> This client connects to a local Aztec node and uses a local test
            account for gameplay. No external wallet setup is required.
          </>
        ) : (
          <>
            <Red>There is real money being transacted in-game!</Red> We have initialized a burner
            wallet for you.
          </>
        )}
      </p>
      ...
      {isAztec ? (
        <p>
          Transactions are submitted to the Aztec node for you. Any fee sponsorship is handled by
        ...
        <>
          ...
          <p>
            To ensure the safety of your balance, <White>we require you to enable popups</White> so
            that all transactions may be confirmed by you. Note that you can disable popups for
            small transactions in settings. <Icon type={IconType.Settings} />
          </p>
        </>
      )}
      <p>
...
function OnboardStorage({ advance }: { advance: () => void }) {
  ...
  return (
    <StyledOnboardingContent>
      <p>
        The game stores <White>home coordinates</White> and <White>map data</White> in your
        browser's local storage / cache.{' '}
        <Red>If you clear your browser history, you risk losing your local data!</Red>
      </p>
      <p>
        {isAztec ? (
          <>
            Your Aztec account is managed by the node. Your{' '}
            <White>home coordinates</White> are still important for recovery on a new device.
          </>
        ) : (
          <>
            Your <White>private key and home coordinates</White> act as your password. You can use
            them to access your Dark Forest account on other browsers, or to continue playing if
  ...
  return (
    <StyledOnboardingContent>
      <p>
        ...
        <TextPreview
          text={sKey ?? 'Not available in Aztec mode'}
          focusedWidth={'150px'}
