...
export class InitialGameStateDownloader {
  ...
  async download(
    ...
    const loadedTouchedPlanetIds = contractsAPI.getTouchedPlanetIds(planetIdsLoadingBar);

    const revealedProgress = (fraction: number) => {
      revealedPlanetsLoadingBar(fraction);
      revealedPlanetsCoordsLoadingBar(fraction);
    };
    const loadedRevealedCoords = contractsAPI.getRevealedPlanetsCoords(revealedProgress);
    const claimedCoordsMap = new Map<LocationId, ClaimedCoords>();

    const allTouchedPlanetIds = Array.from(
      new Set([
        ...storedTouchedPlanetIds,
        ...(await loadedTouchedPlanetIds),
        ...Array.from(minedPlanetIds),
      ])
    );
    ...
    if (planetsToLoad.length === 0 && allTouchedPlanetIds.length > 0) {
...
const tips = [
  ...
  'Withdrawing an artifact (via a Spacetime Rip) gives you full control of that artifact as an Aztec-native NFT. You can deposit artifacts you have withdrawn back into the universe via Spacetime Rips.',
