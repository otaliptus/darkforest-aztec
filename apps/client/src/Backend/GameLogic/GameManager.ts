...
class GameManager extends EventEmitter {
   ...
   */
  private readonly ethConnection: AztecConnection;
  ...
  static async create({
    ...
    // Skip Ethereum tx persistence flow for Aztec.
  ...
  private onTxSubmit(tx: Transaction): void {
    ...
    this.terminal.current?.printLink(
      `${shortHash}`,
      () => {
        if (hash) window.open(`${BLOCK_EXPLORER_URL}/${hash}`);
      },
  ...
  private onTxConfirmed(tx: Transaction) {
    ...
    const shortHash = hash ? hash.slice(0, 6) : 'pending';
    const timings = (tx as Transaction & { __dfTimings?: TxTimings }).__dfTimings;
    const confirmMs = formatDuration(timings?.confirmMs);
    const totalMs = formatDuration(timings?.totalMs);
    ...
    this.terminal.current?.println(
      `) confirmed${confirmMs ? ` in ${confirmMs}` : ''}${totalMs ? ` (total ${totalMs})` : ''}`,
  ...
  async getSignedTwitter(twitter: string): Promise<string> {
    throw new Error('Twitter verification is not supported in the Aztec client.');
  }
  ...
  public async revealLocation(planetId: LocationId): Promise<Transaction<UnconfirmedReveal>> {
    try {
      ...
      const getArgs = async () => {
        const args = [planet.location.coords.x, planet.location.coords.y];
        this.terminal.current?.println('REVEAL: prepared Aztec args:', TerminalTextStyle.Sub);
        this.terminal.current?.println(JSON.stringify(args), TerminalTextStyle.Sub);
  ...
  public async joinGame(beforeRetry: (e: Error) => Promise<boolean>): Promise<void> {
    try {
      ...
      const getArgs = async () => {
        ...
        const args = [planet.location.coords.x, planet.location.coords.y, radius];
        this.terminal.current?.println('INIT: prepared Aztec args:', TerminalTextStyle.Sub);
        this.terminal.current?.println(JSON.stringify(args), TerminalTextStyle.Sub);
        this.terminal.current?.newline();
        return args;
      };
  ...
  public async findArtifact(
    ...
    try {
      ...
      const txIntent: UnconfirmedFindArtifact = {
        ...
        args: Promise.resolve([planet.location.coords.x, planet.location.coords.y]),
      };
  ...
  public async withdrawSilver(
    ...
    try {
      ...
      const txIntent: UnconfirmedWithdrawSilver = {
        ...
        contract: this.contractsAPI.contract,
        args: Promise.resolve([locationIdToDecStr(locationId), amount]),
        locationId,
        amount,
      };
  ...
  public setPlanetEmoji(locationId: LocationId, emojiStr: string) {
    throw new Error('Planet emojis are not supported in the Aztec client.');
  }
  ...
  private async submitPlanetMessage(
    ...
    throw new Error('Planet messages are not supported in the Aztec client.');
  }
    ...
    try {
      ...
      const getArgs = async (): Promise<unknown[]> => {
        const args: unknown[] = [
          ...
          shipsMoved,
          silverMoved,
          artifactMoved ? artifactIdToDecStr(artifactMoved) : '0',
          abandoning,
        ];

        this.terminal.current?.println('MOVE: prepared Aztec args:', TerminalTextStyle.Sub);
        this.terminal.current?.println(JSON.stringify(args), TerminalTextStyle.Sub);
        this.terminal.current?.newline();
  ...
  public loadContract<T extends Contract>(
    ...
    throw new Error('Loading arbitrary contracts is not supported in the Aztec client.');
  }
